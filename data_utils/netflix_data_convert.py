...
def create_NETFLIX_data_timesplit(all_data, train_min, train_max, test_min, test_max):
    """
    ...
    for userId, userRatings in all_data.items():
        time_sorted_ratings = sorted(userRatings, key=lambda x: x[2])
    ...
    for text_file in text_files:
        with open(text_file, "r") as f:
            ...
            for rating in lines[1:]:
                ...
                ts = int(
                    time.mktime(
                        datetime.datetime.strptime(parts[2], "%Y-%m-%d").timetuple()
                    )
                )
