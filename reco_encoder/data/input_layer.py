...
class UserItemRecDataProvider:
    def __init__(self, params, user_id_map=None, item_id_map=None):
        ...
        self._extension = (
            ".txt" if "extension" not in self.params else self.params["extension"]
        )
        ...
        if not (self._major == "items" or self._major == "users"):
            raise ValueError(
                "Major must be 'users' or 'items', but got {}".format(self._major)
            )
        ...
        self._delimiter = (
            "\t" if "delimiter" not in self.params else self.params["delimiter"]
        )
        ...
        for source_file in src_files:
            with open(source_file, "r") as src:
                for line in src.readlines():
                    ...
                    if len(parts) < 3:
                        raise ValueError(
                            "Encountered badly formatted line in {}".format(source_file)
                        )
    ...
    def iterate_one_epoch(self):
        ...
        while e_ind < len(keys):
            ...
            for ind in range(s_ind, e_ind):
                ...
                local_ind += 1
            i_torch = paddle.to_tensor(data=[inds1, inds2], dtype="int64")
            v_torch = paddle.to_tensor(data=vals, dtype="float32")
            mini_batch = paddle.sparse.sparse_coo_tensor(
                i_torch, v_torch, shape=[self._batch_size, self._vector_dim]
            )
    ...
    def iterate_one_epoch_eval(self, for_inf=False):
        ...
        while s_ind < len(keys):
            ...
            i_torch = paddle.to_tensor(data=[inds1, inds2], dtype="int64")
            v_torch = paddle.to_tensor(data=vals, dtype="float32")
            src_i_torch = paddle.to_tensor(data=[src_inds1, src_inds2], dtype="int64")
            src_v_torch = paddle.to_tensor(data=src_vals, dtype="float32")
            mini_batch = (
                paddle.sparse.sparse_coo_tensor(
                    i_torch, v_torch, list([1, self._vector_dim])
                ),
                paddle.sparse.sparse_coo_tensor(
                    src_i_torch, src_v_torch, list([1, self._vector_dim])
                ),
            )
            s_ind += 1
            if not for_inf:
